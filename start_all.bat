...
start "ML API" cmd /k "cd AI_Model_CSR_Denial_Knowledge_Bot && py enhanced_api.py"
...
if exist "%~dp0Backend_CSR_Denial_Knowledge_Bot\.env" (
  ...
  for /f "usebackq tokens=1,* delims==" %%A in (`findstr /R "^[A-Za-z_][A-Za-z0-9_]*=" "%~dp0Backend_CSR_Denial_Knowledge_Bot\.env"`) do (
    if not "%%B"=="" set "%%A=%%B"
  )
)

start "Backend" cmd /c "cd Backend_CSR_Denial_Knowledge_Bot && title Backend && mvn spring-boot:run -Dspring-boot.run.arguments=--app.disable-auth=true > backend.log 2>&1"
